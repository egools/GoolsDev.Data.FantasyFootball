...
{
    public static class PFRUtility
    {
        ...
        private static Func<string, string> GetAbbr = (string str) => Regex.Replace(str, "/teams/(.*)/\\d{4}\\.htm", "$1");
        public static IEnumerable<PFRGame> GetNFLRegularSeasonSchedule(int year) => GetNFLGames(year, $"#games th[data-stat='week_num']");
        public static IEnumerable<PFRGame> GetRegularSeasonGames(int year, int week) => GetNFLGames(year, $"#games th[data-stat='week_num'][csk='{week}']");
        private static IEnumerable<PFRGame> GetNFLGames(int year, string selector)
        {
            var document = AngleSharpHelper.GetDocumentFromUrl($"https://www.pro-football-reference.com/years/{year}/games.htm");
            ...
            foreach (var weekElement in weekElements)
            {
                if (int.TryParse(weekElement.Text(), out _))
                    yield return PFFGameFromRow(weekElement.ParentElement);
            }
        }
        ...
        public static PFRPlayer FindPlayerInfo(string playerName)
        {
            var urlEncodedName = HttpUtility.UrlEncode(playerName);
            var playerDoc = AngleSharpHelper.GetDocumentFromUrl($"https://www.pro-football-reference.com/search/search.fcgi?search={urlEncodedName}");
            ...
            if(hasMutlipleTeams)
            {
                playerDoc  = AngleSharpHelper.GetDocumentFromUrl(player.BaseUri.Replace(".htm", "/gamelog/"));
